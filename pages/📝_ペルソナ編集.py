...
with tab_create:
    ...
    # 保存済みプロフィールが存在するが、このセッションではまだ読み込んでいない場合に通知
    if manager.profile is None and manager.has_saved_profile():
        st.info(
            "💾 前回保存したペルソナプロフィールが見つかりました。"
            "読み込む場合はボタンを押してください。"
        )
        if st.button("📂 保存済みペルソナを読み込む"):
            manager.load_profile_from_disk()
    ...
    if manager.profile:
        ...
        if st.button("🗑️ ペルソナをリセット", type="secondary"):
            manager.delete_profile()  # メモリ + ディスクの両方を削除
            st.session_state.persona_manager = manager
            st.session_state.rewrite_result = None
            st.rerun()
